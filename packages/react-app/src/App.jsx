...
import { Alert, Button, Col, Menu, Row, List, Input, Divider, Tooltip } from "antd";
...
function App(props) {
  ...
  // retreive the reward rate per block from contract
  const rewardRatePerBlock = useContractReader(readContracts, "Staker", "rewardRatePerBlock");
  console.log("⏳ Reward Rate Per Block:", rewardRatePerBlock);
  ...
  */
  ...
  const updateStake = newValue => {
    if (typeof newValue !== "undefined" && newValue > 0 && newValue <= 10) {
      ...
      <BrowserRouter>
        ...
        <Switch>
          <Route exact path="/">
            ...
            <div style={{ padding: 8, justifyContent: 'center', flex: 1 }}>
              <Input
                style={{ width: 200 }} 
                size="medium"
                placeholder={"ETH amount to stake"}
                onChange={e => updateStake(e.target.value)}
                suffix={
                  <Tooltip title="Click to stake ETH.">
                    <Button
                      onClick={() => {
            ...
            <div style={{ padding: 8 }}>
              ...
              <Button
                type={"default"}
                onClick={() => {
                  tx(writeContracts.Staker.restartTime());
                }}
              >
                🕛 Restart
              </Button>
              <Button
                type={"default"}
                onClick={() => {
                  tx(writeContracts.Staker.unStake());
                }}
              >
                🏧 Unstake
              </Button>
